...
export const EditCharacter = () => {
    ...
    useEffect(() => {
        ...
        getCharacter(id).then(c => setForm({
            name: c.name ?? "",
            race_id: c.race?.id ?? "",
            ...
            hp_max: c.hp_max ?? 1,
            hp_current: c.hp_current ?? 1,
            background: c.background ?? "",
            alignment: c.alignment ?? "",
            backstory: c.backstory ?? "",
            class_levels: (c.class_levels ?? []).map(cl => ({
                dnd_class_id: cl.dnd_class?.id ?? "",
                level: cl.level ?? 1,
    ...
    const totalLevel = form?.class_levels?.reduce((sum, cl) => sum + Number(cl.level || 0), 0) ?? 0
    ...
    const handleSubmit = (e) => {
        ...
        updateCharacter(id, payload).then(() => navigate(`/characters/${id}`))
    }
    ...
    return (
        <main className="page-shell max-w-3xl mx-auto px-4 pt-24 pb-16">
            <section className="scroll-card p-8">
                <h1 className="text-3xl text-center mb-2">Amend the Chronicle</h1>
                <p className="text-center text-ink-700 italic mb-2">Edit your hero's tale</p>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col gap-3">
                            {form.class_levels.map((cl, i) => {
                                ...
                                // If existing subclass isn't in the current options list, include it to avoid data loss
                                const optionsToRender = (!sub.disabled && cl.subclass && !sub.options.includes(cl.subclass))
                                    ? [cl.subclass, ...sub.options]
                                    : sub.options
                                return (
                                    <div key={i} className="grid grid-cols-12 gap-2 items-end">
                                        ...
                                        <div className="col-span-5 flex flex-col gap-1">
                                            ...
                                            >
                                                ...
                                                {optionsToRender.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                            </select>
                                        </div>
                    ...
                    <div>
                        ...
                        <fieldset className="flex flex-col gap-1">
                            ...
                            <textarea className="form-surface" rows="5" value={form.backstory} onChange={change("backstory")} />
                        </fieldset>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 mt-2">
                        <button type="button" onClick={() => navigate(`/characters/${id}`)} className="btn-ghost">Cancel</button>
